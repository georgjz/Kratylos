...
###### Ambiguity
A context-free grammar is [[Ambiguity|ambiguous]] iff for string $s \in L(G)$ there are two or more derivations (aka, a context-free grammar is unambiguous if each string's derivation is unique). Undecidable.
...
###### Reachability
A nonterminal $X$ is [[Reachability|reachable]] if there is a derivation $S\;\xRightarrow{\ast}\;\alpha X \beta$, where $\alpha$ and $\beta$ are strings of nonterminals and terminals, and $S$ the start symbol. Decidable.

###### Usability
A nonterminal $X$ is [[Usability|useless]] if it is [[Productivity|unproductive]] or [[Reachability|unreachable]]. Decidable by extension.

###### Nullability
A nonterminal $X$ is [[Nullability|nullable]] if there is a derivation $X\;\xRightarrow{\ast}\;\epsilon$. Decidable.
